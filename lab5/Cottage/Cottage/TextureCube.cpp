...
TextureCube::TextureCube(glm::vec3 position, float size)
	:m_position(position)
	,m_size(size)
{
	for (std::size_t i = 0; i < 8; ++i)
	{
		...
		{
			m_vertices[i][j] = (VERTICES[i][j] + 1) * m_size * 0.5f + m_position[j];
		}
	}
}
...
void TextureCube::SetTexture(TextureHandle& texture)
{
	m_texture = texture;
}
